...
export class V2Quoter {
  ...
  async calBuyAmount(buyToken: string, sellToken: string, buyTax: number, sellTax: number, sellAmount: string, dexData: string): Promise<BigNumber> {
    return toBN(await this.contract.calBuyAmount(
      buyToken,
      sellToken,
      buyTax,
      sellTax,
  ...
  async calSellAmount(buyToken: string, sellToken: string, buyTax: number, sellTax: number, buyAmount: string, dexData: string): Promise<BigNumber> {
    return toBN(await this.contract.calSellAmount(
      buyToken,
      sellToken,
      buyTax,
      sellTax,
  ...
  async getUniV2PairInfo(token0Address: string, token1Address: string, token0Decimal: number, token1Decimal: number, dexName: string, dexData: string, factory: string) {
  ...
  async checkUniV2LiqLimit(pair: Pair, sellToken: string, borrowing: BigNumber, poolAddress: string, factory: string) {
    ...
    if (borrowing.multipliedBy(toBN(100)).dividedBy(liqInPoolByETH).comparedTo(ratio) > 0) {
      ...
      const lPoolContract = new Contract(
        poolAddress,
        [
          Abi.borrowBalanceCurrent
        ],
        this.signer
      )
      const borrowBalanceCurrentBN = toBN(await lPoolContract.borrowBalanceCurrent(this.signer.address))

      const borrowBeforeByETH = borrowBalanceCurrentBN.dividedBy(toBN(10).pow(sellToken == pair.token0Address ? pair.token0Decimal : pair.token1Decimal));
    ...
    if (throwBorrowLimitEx) {
      throw ({ message: "The maximum borrowing of total position must be Lower than a 5% of liquidity on DEX. To initiate trade, please adjust current leverage." });
    }
  }
}