...
export class TradeCalculator {
  ...
  constructor(config: TradeCalculatorConfig) {
    ...
    this.oplContract = new Contract(this.chainAddress.oplAddress, OplAbiJson.abi, config.signer)
  ...
  async getPoolInfo(poolAddress: string, decimal: number): Promise<PoolInfo> {
    ...
    const borrowInterest = toBN(borrowRatePerBlock.toString())
      .multipliedBy(toBN(this.blocksPerYear))
  ...
  async getMarketInfo(marketId: number): Promise<MarketInfo | null> {
    ...
    return {
      ...
      discountLeverFeesRate:
        market.priceUpdator == this.signer.address
          ? this.getPriceUpdaterDiscount(toBN(market.feesRate))
          : toBN(market.feesRate).toString(),
      priceUpdator: market.priceUpdator,
    }
  }

  async calculateBorrowing(
    pair: Pair,
    tradeInfo: TradeInfo,
    discountLeverFeesRate: string,
  ): Promise<BorrowToTradeResult> {
    ...
    const token0PriceOfToken1 = await this.tradeRouter.getTokenQuotePrice(
      token0Address,
      token1Address,
      token0Decimal,
      token1Decimal,
    ...
    if (tradeInfo.longToken !== tradeInfo.depositToken) {
      ...
      borrowing = this.calculateBorrowingAmount(
        depositAmountBN,
        token0PriceOfToken1,
        levelMinusOne,
        tradeInfo.depositToken,
      )
      leverTotalAmount = depositAmountBN.plus(
        depositAmountBN
          .multipliedBy(levelMinusOne)
          .multipliedBy(toBN(feeRatePrecision).minus(borrowTranFee))
          .dividedBy(toBN(feeRatePrecision)),
      )
    }
    const discountLeverFees = leverTotalAmount.multipliedBy(discountLeverFeesRate).dividedBy(toBN(10000))
    swapTotalAmount = this.calculateSwapTotalAmount(
      borrowing,
      borrowTranFee,
      depositAmountBN,
      depositTranFee,
  ...
  calculateLeverTotalAmount(borrowing: BigNumber, depositAmountBN: BigNumber, depositTranFee: BigNumber) {
    return borrowing.plus(
      depositAmountBN.multipliedBy(toBN(feeRatePrecision).minus(depositTranFee)).dividedBy(toBN(feeRatePrecision)),
    )
  }

  calculateBorrowingAmount(
    depositAmountBN: BigNumber,
    token0PriceOfToken1: BigNumber,
    levelMinusOne: BigNumber,
    depositToken: number,
  ) {
    return depositToken === 0
      ? depositAmountBN.multipliedBy(token0PriceOfToken1).multipliedBy(levelMinusOne)
  ...
  calculateSwapTotalAmount(
    ...
    return isLongTokenEqualDepositToken
      ...
      : borrowing
          ...
          .plus(
            depositAmountBN
              .multipliedBy(toBN(feeRatePrecision).minus(depositTranFee))
              .dividedBy(toBN(feeRatePrecision)),
          )
  ...
  async checkNeedToUpdatePrice(
    ...
    for (const [dex, dexQuoteRes] of dexQuoteMap) {
      if (dex !== oneInch) {
        ...
        const marketValueC = toBN(dexQuoteRes.held)
          .multipliedBy(priceCAvgPriceHAvgPrice.cAvgPrice)
          .dividedBy(toBN(10).pow(priceCAvgPriceHAvgPrice.decimals))
        dexQuoteRes.shouldUpdatePrice = marketValueC
          .minus(borrowToTradeRes.borrowing)
          .multipliedBy(10000)
          .dividedBy(borrowToTradeRes.borrowing)
          .lt(marketInfo.marginLimit)
        //v3 waiting
        ...
        if (
          dexQuoteRes.priceImpact &&
          dexQuoteRes.shouldUpdatePrice &&
          toBN(dexQuoteRes.priceImpact).abs().gt(toBN(10))
        ) {
          dexQuoteRes.shouldUpdatePrice = false
        }
      }
  ...
  async getCAvgPriceNHAvgPrice(pair: Pair, tradeInfo: TradeInfo, dexQuoteRes: tradeQuoteResult) {
    const priceCAvgPriceHAvgPrice = await this.queryHelperContract.calPriceCAvgPriceHAvgPrice.staticCall(
      this.chainAddress.oplAddress,
      pair.marketId,
      tradeInfo.buyToken,
      tradeInfo.sellToken,
      this.tWap,
      dexQuoteRes.dexCallData,
    )
    ...
    return {
      cAvgPrice:
        tradeInfo.longToken == 0
          ? toBN(cAvgPrice).multipliedBy(toBN(10).pow(pair.token0Decimal - pair.token1Decimal))
          : toBN(cAvgPrice).multipliedBy(toBN(10).pow(pair.token1Decimal - pair.token0Decimal)),
      hAvgPrice:
        tradeInfo.longToken == 0
          ? toBN(hAvgPrice).multipliedBy(toBN(10).pow(pair.token0Decimal - pair.token1Decimal))
