...
if (tradePreviewRes) {
  ...
  if (dexInfo && dexInfo.dexCallData) {
    const erc20Abi = [Abi.erc20Approve]

    const oleContract = new ethers.Contract(tradeInfo.buyToken, erc20Abi, signer)
    const approveTx = await oleContract.approve(
      chain.addresses.oplAddress,
      ethers.parseUnits(
        tradeInfo.depositAmount,
    ...
    const marginTradeTx = await marginTradeHelper.openTrade(
      pair,
      tradeInfo,
      dexInfo.minBuyAmount,
      tradePreviewRes.borrowing,
      tradePreviewRes.dex,
      toBN(dexInfo.swapTotalAmountInWei),
      dexInfo.dexCallData,
    )
